...
The app organises notes by tag, not by directory: it lists every note in one flat list and never creates, renames or moves files. Choose a note's location yourself and keep it stable, because moving a file breaks its relative links in both directions. Tags can be added or removed inline in the app, so reread the current file before saving to preserve concurrent edits.
