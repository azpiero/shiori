(function(root){
 ...
 function create({document,invoke,listen,getContext,onBusy,onComplete,Terminal=root.Terminal,FitAddon=root.FitAddon.FitAddon,ResizeObserver=root.ResizeObserver}){
  const $=s=>document.querySelector(s);let term=null,fit=null,session=null,ready=false,starting=false,inputQueue=Promise.resolve(),queuedBytes=0;
  $('#terminalPanel').innerHTML='<div class="terminal-heading"><strong>Terminal</strong><span id="terminalState" role="status"></span><span id="terminalPrompt"></span><div class="terminal-info"><button id="terminalInfo" aria-label="ターミナル情報" aria-expanded="false" aria-controls="terminalContext">ⓘ</button><div id="terminalContext" class="terminal-context" role="region" aria-label="ターミナル情報" hidden><p id="terminalPaths"></p></div></div><button id="terminalHide" aria-label="ターミナルを隠す">×</button></div><div class="terminal-display"><div id="terminalScreen"></div><div id="terminalRestart" class="terminal-restart" hidden><button id="terminalRetry">シェルを再起動</button></div></div>';
  ...
  function input(data){
   const active=session;if(!active||starting)return;
   if(queuedBytes+data.length>65536){state('入力待ちが多いため貼り付けを停止しました');return;}
   queuedBytes+=data.length;
   inputQueue=inputQueue.then(async()=>{try{for(let i=0;i<data.length;i+=4096){if(session!==active)break;await invoke('terminal_write',{sessionId:active.id,data:Array.from(data.slice(i,i+4096))});}}catch(e){state(String(e));}finally{queuedBytes-=data.length;}});
  }
  ...
  async function start(){
   if(!ready||session||!getContext())return;mount();term.reset();session={...getContext(),id:uuid()};const active=session;starting=true;onBusy(true);state('シェルを起動しています…');render();fit.fit();
   try{const info=await invoke('terminal_start',{sessionId:active.id,vaultToken:active.token,rows:Math.max(2,Math.min(200,term.rows)),cols:Math.max(2,Math.min(400,term.cols))});starting=false;if(session===active){active.cwd=info.cwd;render();resize();state('');term.focus();}}
   catch(e){if(session===active){session=null;starting=false;onBusy(false);state(String(e));render();}}
  }
  ...
  $('#showTerminal').onclick=show;$('#terminalRetry').onclick=start;
  ...
  const initialized=Promise.resolve(listen('terminal-output',({payload})=>{
   ...
   if(payload.exit){session=null;starting=false;onBusy(false);state(payload.message);onComplete(active.token);render();return;}
