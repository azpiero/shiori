(function(root){
 ...
 function create({document,getVault,getTheme,onSelect,onStatus,onTag=()=>{},onEditTags=()=>{}}){
  ...
  return {model,frames,open,render,activate,moveHit,syncFocusedFrame,
   ...
   moved(oldPath,newPath,paths){for(const tab of model.all())if(tab.path===oldPath)tab.path=newPath;model.reconcile(getVault().notes);for(const tab of model.all())if(paths.has(tab.path))navigate(tab);render();notify();},
