(function(root){
 ...
 function tree(notes,paths,filtered=false){
  const nodes=new Map(groups(notes,paths).map(group=>[group.path,{...group,children:[]}]));
  const roots=[];for(const node of nodes.values()){const owner=nodes.get(parent(node.path));if(owner)owner.children.push(node);else roots.push(node);}
  function prune(nodes){return nodes.filter(node=>{node.children=prune(node.children);return !filtered||node.notes.length||node.children.length;});}
  return prune(roots);
 }
 ...
 root.ShioriFolders={parent,allowed,folders,groups,tree,canDrop};if(typeof module!=='undefined')module.exports=root.ShioriFolders;
})(globalThis);
