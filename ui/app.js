...
let graphMode=false,graphPage=0,graphScale=1,graphX=0,graphY=0,currentMatches=[];
const metrics={uiReadyMs:0,scanMs:0,lastSwitchMs:0,imeCompositions:0};
const $=s=>document.querySelector(s);
const escape=s=>String(s).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
$('#app').innerHTML=`<header class="topbar"><div class="brand"><img class="logo" src="assets/shiori-icon.png" alt="" width="31" height="31"><div><strong>shiori</strong><div class="caption">A quieter place for your ideas</div></div><span class="pill">表示試作 / 読み取り専用</span></div><div class="actions"><button id="sample">サンプル</button><button id="open" class="primary">フォルダを開く</button><button id="theme" title="ライト／ダーク切替">◐</button><button id="inspect">検証ログ</button></div></header><div class="layout"><aside class="sidebar"><div class="eyebrow">YOUR LIBRARY</div><div class="vault-name"><span id="vaultName">Sample Vault</span><span id="count" class="pill">—</span></div><div class="vault-path" id="root"></div><div class="search"><input id="search" placeholder="ノートを検索…" aria-label="ノートを検索" autocomplete="off"></div><div class="filter-tags" id="tags"></div><div class="list-head"><span>NOTES</span><span id="results"></span></div><div class="notes" id="notes"></div><div class="sidebar-footer">HTMLをそのまま正本に。<br>ノート内のJavaScript・外部資産は停止。<br>この試作はファイルを書き換えません。</div></aside><div class="splitter" id="splitter" role="separator" aria-label="サイドバー幅" aria-orientation="vertical"></div><main class="viewer"><div class="document-toolbar"><div id="breadcrumb" class="breadcrumb">ノートを選択</div><div class="view-switch"><button id="showNote" aria-pressed="true">ノート</button><button id="showGraph" aria-pressed="false">タググラフ</button></div><div id="noteTags" class="note-tags"></div><div class="document-tools"><select id="headings" aria-label="見出しへ移動"><option value="">見出しへ移動</option></select><button id="prev" title="前の検索箇所">↑</button><span id="hitCount" class="caption">—</span><button id="next" title="次の検索箇所">↓</button><button id="textFragment" title="iframeではText fragmentは対象外のため比較用">Text fragment検証</button><button id="reload">再読込</button></div></div><div class="notice" id="notice"><span id="noticeText">ファイルが変更されました。再読み込みで反映できます。</span><button id="update">更新を反映</button></div><div class="frame-wrap"><iframe id="noteFrame" sandbox="" referrerpolicy="no-referrer" title="HTMLノート（隔離表示）"></iframe><div id="loading" class="spinner">サンプルを読み込んでいます…</div></div><section id="graphPanel" hidden><div class="graph-toolbar"><div><strong>タグでつながるノート</strong><div id="graphSummary" class="graph-summary"></div></div><div class="graph-actions"><button id="graphPrev" aria-label="前のグラフ">←</button><button id="graphNext" aria-label="次のグラフ">→</button><button id="zoomOut" aria-label="縮小">−</button><button id="zoomIn" aria-label="拡大">＋</button><button id="graphReset">全体表示</button></div></div><p class="graph-help">大きい点＝タグ · 小さい点＝ノート。タグを選んで絞り込み、ノートを選んで本文へ。ドラッグで移動。点にカーソルを重ねるとタイトルを表示。</p><svg id="graphSvg" viewBox="0 0 1000 740" aria-label="タグとノートの関係" role="group"></svg><div id="graphEmpty" class="empty" hidden>一致するノートがありません</div></section><section id="diagnostics" class="diagnostics"><h3>表示試作の検証ログ</h3><p>配信ログはRust側が扱ったリクエストです。CSPがWebView内で止めた外部通信はここには届きません。</p><pre id="log"></pre></section></main></div><footer class="statusbar"><span id="status">準備中</span><span id="timing">Tauri + Rust · sandboxed iframe · WKWebView</span></footer>`;
...
function renderList(){
 ...
 $('#notes').innerHTML=matches.map(n=>`<button class="note ${n.path===selected?'active':''}" data-path="${escape(n.path)}"><div class="note-title"><span class="note-icon">▤</span>${escape(n.title)}</div><div class="note-path">${escape(n.path)}</div><div class="list-note-tags">${n.tags.map(t=>`<span>#${escape(t)}</span>`).join('')||'<span class="untagged">タグなし</span>'}</div></button>`).join('')||'<div class="empty">一致するノートがありません</div>';
 ...
 $('#tags').querySelectorAll('button').forEach(b=>b.onclick=()=>{setTag(b.dataset.tag);});
}
function selectedInfo(){const n=vault.notes.find(n=>n.path===selected);$('#breadcrumb').textContent=selected;$('#noteTags').innerHTML=(n?.tags||[]).map(t=>`<button class="tag" data-tag="${escape(t)}">#${escape(t)}</button>`).join('')||'<span class="untagged">タグなし</span>';$('#noteTags').querySelectorAll('button').forEach(b=>b.onclick=()=>{setTag(b.dataset.tag);setView(true);});$('#headings').innerHTML='<option value="">見出しへ移動</option>'+(n?.headings||[]).map(h=>`<option value="${escape(h.id)}">${escape(h.text)}</option>`).join('');renderList();}
function openNote(path,anchor=''){
 if(!vault)return;setView(false);selected=path;hit=0;hits=0;selectedInfo();switching=performance.now();$('#loading').classList.remove('hidden');
...
async function load(path=null){status('HTMLを解析しています…');$('#loading').classList.remove('hidden');try{vault=await invoke('open_vault',{path});metrics.scanMs=vault.scan_ms;selected='';graphPage=0;tag='';query='';$('#search').value='';$('#root').textContent=vault.root;$('#root').title=vault.root;$('#vaultName').textContent=vault.root.split('/').pop();$('#count').textContent=vault.notes.length;$('#notice').classList.remove('show');changed=false;renderList();if(vault.notes.length)openNote(vault.notes[0].path);else{$('#noteTags').textContent='';$('#breadcrumb').textContent='ノートがありません';$('#noteFrame').src='about:blank';$('#loading').classList.add('hidden');status('HTMLノートがありません');}if(vault.errors.length)status(`${vault.errors.length}件の読み取りエラー（検証ログ参照）`);}catch(e){status(String(e));$('#loading').classList.add('hidden');}}
...
let timer;function search(){if(composing)return;clearTimeout(timer);timer=setTimeout(()=>{query=$('#search').value;graphPage=0;renderList();},120);}
$('#search').addEventListener('input',search);
$('#search').addEventListener('keydown',e=>{if(e.key==='Enter'&&!e.isComposing&&!composing){query=$('#search').value;graphPage=0;renderList();const first=$('#notes button');if(first)openNote(first.dataset.path);}});
...
async function refresh(){try{const old=selected,wasGraph=graphMode;vault=await invoke('refresh_vault');metrics.scanMs=vault.scan_ms;$('#count').textContent=vault.notes.length;changed=false;$('#notice').classList.remove('show');renderList();if(vault.notes.length){openNote(vault.notes.some(n=>n.path===old)?old:vault.notes[0].path);}else{selected='';$('#noteTags').textContent='';$('#noteFrame').src='about:blank';$('#breadcrumb').textContent='ノートがありません';$('#loading').classList.add('hidden');}setView(wasGraph);}catch(e){status(String(e));}}
...
function setTag(value){tag=value;graphPage=0;renderList();}
function setView(graph){graphMode=graph;$('#graphPanel').hidden=!graph;$('.frame-wrap').hidden=graph;$('.document-tools').hidden=graph;$('#showGraph').setAttribute('aria-pressed',String(graph));$('#showNote').setAttribute('aria-pressed',String(!graph));if(graph)renderGraph();}
...
function activateGraph(e){if(e.type==='keydown'&&!['Enter',' '].includes(e.key))return;if(e.type==='click'&&suppressGraphClick){suppressGraphClick=false;return;}const node=e.target.closest('.graph-node');if(!node)return;e.preventDefault();if(node.dataset.kind==='tag')setTag(node.dataset.value);else openNote(node.dataset.value);}
