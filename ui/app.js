...
function renderList(){
 ...
 const renderFolder=({path,notes,children})=>{
  ...
  return `<section class="folder-group"><div class="folder-heading" data-folder-heading="${escape(path)}">${renaming?editor():`<button class="folder-row" data-folder="${escape(path)}" aria-expanded="${expanded}" aria-controls="folder-notes-${id}" title="${escape(path)}"><span aria-hidden="true">${expanded?'▾':'▸'}</span><svg class="tree-icon folder-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M3 7V5h6l2 2h10v13H3z"/></svg><span class="folder-name">${escape(path.split('/').pop())}</span></button>`}</div><div class="folder-children" id="folder-notes-${id}" ${expanded?'':'hidden'}>${folderEdit?.mode==='create'&&folderEdit.path===path?editor():''}${children.map(renderFolder).join('')}${notes.map(n=>`<article class="note ${n.path===selected?'active':''}"><button class="note-open" draggable="true" aria-describedby="folderMoveHelp" title="${escape(n.path)}" aria-label="${escape(n.title)} — ${escape(n.path)}" data-path="${escape(n.path)}" ${n.path===selected?'aria-current="true"':''}><svg class="tree-icon document-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M5 3h9l5 5v13H5zM14 3v6h5M8 13h8M8 17h6"/></svg><span class="note-title">${escape(n.title)}</span></button></article>`).join('')}</div></section>`;
 };
