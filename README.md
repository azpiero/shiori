...
The user's default shell starts with its normal environment/login setup. Install and authenticate tools as usual. Hiding the panel preserves the shell; the **ⓘ** popover shows the full, copyable HTML destination and working directory and contains **シェルを終了**. After the shell exits or startup fails, use **シェルを再起動** in the terminal area to start a new session. Escape or an outside click dismisses the popover. Close the shell before switching vaults, then reopen it to receive the new destination. A normal vault refresh stays available while the terminal is open, and external edits trigger **更新を反映** without an automatic scroll reset.
